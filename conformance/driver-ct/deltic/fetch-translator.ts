...
//
// The runtime itself is consumed as pinned raw SOURCE over those import
// maps (with deno.lock carrying the integrity hashes, enforced by
// --frozen), not as the release's prebuilt `deltic-embedder.mjs` bundle:
// the bundle would be a second, separately-pinned copy of the same
// modules, and the module-identity constraint below wants exactly one.
//
...
// directory plus `deno cache src/mod.ts tests/families_test.ts` in
